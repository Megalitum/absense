...
namespace BindSense.cs
{
    class HandsRecognition
    {
        ...
        public HandsRecognition(TestForm form)
        {
        ...
        public static extern IntPtr FindWindow(string lpclassname, string lpWindowMain);
        [DllImport("USER32.DLL")]
        public static extern bool SetForegroundWindow(IntPtr hwin);
        /* Displaying current frame gestures */
        private static int lastFrame = -30;
        private void DisplayGesture(PXCMHandData handAnalysis, int frameNumber)
        {
            if (frameNumber <= lastFrame + 5)
            {
                return;
            }
            ...
            for (int i = 0; i < firedGesturesNumber; i++)
            {
                ...
                if (handAnalysis.QueryFiredGestureData(i, out gestureData) == pxcmStatus.PXCM_STATUS_NO_ERROR)
                {
                    ...
                    else if (bodySideType == PXCMHandData.BodySideType.BODY_SIDE_RIGHT)
                    {
                        ...
                        //if(gestureData.name == "swipe_left")
                        //SendKeys.SendWait("{LEFT}");

                        //if (gestureData.name == "swipe_right")
                        //    SendKeys.SendWait("{RIGHT}");

                        //if (gestureData.name == "swipe_up")
                        //    SendKeys.SendWait("{UP}");

                        //if (gestureData.name == "swipe_down")
                        //    SendKeys.SendWait("{DOWN}");

                        // right_key_event[gestureData.name].binder();
        ...
        public void SimplePipeline()
        {
            ...
            /*
            var touchLessStatus = instance.EnableTouchlessController();
            if (touchLessStatus != pxcmStatus.PXCM_STATUS_NO_ERROR)
            {
                form.UpdateStatus("Failed to enable TouchlessCotroller");
            }
            var handler = new PXCMSenseManager.Handler();
            instance.Init(handler);
            var ptc = instance.QueryTouchlessController();
            ...
            if (handAnalysis != null && instance.Init(handler) == pxcmStatus.PXCM_STATUS_NO_ERROR)
            {
                ...
                handConfiguration.DisableGesture("spreadfingers");
                handConfiguration.DisableGesture("wave");
                handConfiguration.DisableGesture("two_fingers_pinch_open");
                handConfiguration.DisableGesture("full_pinch");
                handConfiguration.ApplyChanges();
                while (!form.stop)
                {
